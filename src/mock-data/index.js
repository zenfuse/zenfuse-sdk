...
const getDirs = (folderPath) => {
    try {
        ...
        for (const dir in dirs) {
            ...
            const isDir = fs.statSync(path.join(__dirname, currentFolderPath)).isDirectory();
            ...
            if (isDir) {
                ...
                pathArray.map((folder, index) => {
                    if (index === pathArray.length - 1 && folder.indexOf('.js') > -1) {
                        ...
                        const fileContent = require(path.join(__dirname, currentFolderPath));
                        ...
                        allElements = R.mergeDeepWith(merge, allElements, arrayToAllElements);
                    }
                });
            }
        }
